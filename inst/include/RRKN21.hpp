...
class RRKN21{
  ...
  Eigen::VectorXd q_high_;
  Eigen::VectorXd v_high_;
...
public:
  ...
  RRKN21() : absTol_(1.0e-3), relTol_(1.0e-3), eps_(0.5) {}
  ...
  void newEpsAfterAccept(const Eigen::VectorXd& nextU) {
    double ufacq = std::abs((nextU(0)-1.0/3.0)/(U_(0)-1.0/3.0));
    double ufacv = std::abs((nextU(0)-0.5)/(U_(0)-0.5));
  ...
  bool step(const Eigen::VectorXd& U){
    ...
    //std::cout << "RRKN21 step, U: " << U << " eps : " << eps_ << std::endl;
    // new q
    ...
    ys_.col(2).segment(sor_.pos_first_,sor_.pos_last_-sor_.pos_first_+1) = // q
      ...
      (0.5*epssq)*(force_.col(1).segment(sor_.vel_first_,sor_.vel_last_-sor_.vel_first_+1));
    // v
    ys_.col(2).segment(sor_.vel_first_,sor_.vel_last_-sor_.vel_first_+1) = // v
      ys_.col(0).segment(sor_.vel_first_,sor_.vel_last_-sor_.vel_first_+1) + // v0
      (eps_)*(force_.col(1).segment(sor_.vel_first_,sor_.vel_last_-sor_.vel_first_+1));
    // r
    ys_.col(2).segment(sor_.int_first_,sor_.int_last_-sor_.int_first_+1) = // r
      ys_.col(0).segment(sor_.int_first_,sor_.int_last_-sor_.int_first_+1) + // r0
      (eps_)*(force_.col(1).segment(sor_.int_first_,sor_.int_last_-sor_.int_first_+1));
    ...
    q_high_ = ys_.col(0).segment(sor_.pos_first_,sor_.pos_last_-sor_.pos_first_+1) + // q0
      (eps_)*(ys_.col(0).segment(sor_.vel_first_,sor_.vel_last_-sor_.vel_first_+1)) + // h*v0
      (epssq*(3.0+uu)/12.0)*(force_.col(0).segment(sor_.vel_first_,sor_.vel_last_-sor_.vel_first_+1)) +
      (epssq/12.0)*(force_.col(1).segment(sor_.vel_first_,sor_.vel_last_-sor_.vel_first_+1)) +
      (epssq/(2.0-uu)/12.0)*(force_.col(2).segment(sor_.vel_first_,sor_.vel_last_-sor_.vel_first_+1));
    // higer order velocity
    v_high_ = ys_.col(0).segment(sor_.vel_first_,sor_.vel_last_-sor_.vel_first_+1) + // v0
      (eps_*(2.0+uu)/6.0)*(force_.col(0).segment(sor_.vel_first_,sor_.vel_last_-sor_.vel_first_+1)) +
      (eps_/6.0)*(force_.col(1).segment(sor_.vel_first_,sor_.vel_last_-sor_.vel_first_+1)) +
      (eps_*(3.0-uu)/6.0)*(force_.col(2).segment(sor_.vel_first_,sor_.vel_last_-sor_.vel_first_+1));
    ...
    for(std::size_t i = sor_.pos_first_; i<= sor_.pos_last_; i++){
      stepErr_q_ = std::max(stepErr_q_,
                            std::abs(q_high_.coeff(i-sor_.pos_first_)-ys_.col(2).coeff(i))/tmpVec_.coeff(i));
    }
    stepErr_v_ = 0.0;
    for(std::size_t i = sor_.vel_first_; i<= sor_.vel_last_; i++){
      stepErr_v_ = std::max(stepErr_v_,
                            std::abs(v_high_.coeff(i-sor_.vel_first_)-ys_.col(2).coeff(i))/tmpVec_.coeff(i));
    }
  ...
  inline bool event(const rootInfo& rootOut){
    ...
    //std::cout << "ode.event done" << std::endl;
    ...
    //std::cout << "step.event done" << std::endl;
