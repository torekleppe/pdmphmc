...
#' pair plot from pdphmc output
#'
#' @description pair plot of pdmphmc output
#' @param object S4 class of type `run-output` typically generated by function `run`
#' @param which.par.x character vector of parameter/generated names on x-axis
#' @param which.par.y character vector of parameter/generated names on y-axis
#' @export
pair.plot<-function(object,which.par.x=NULL,which.par.y=NULL){
  if(!identical(methods::is(object),"run-output")) stop("bad input")
  if(is.null(which.par.x) || is.null(which.par.y)){
    stop("parameter/generated name must be provided")
  }
  dnames <- dimnames(object@pointSamples)[[3]]
  if(is.na(match(which.par.x,dnames))) stop("bad which.par.x")
  if(is.na(match(which.par.y,dnames))) stop("bad which.par.y")


  graphics::par(mfrow=c(1,1))

  plot(as.vector(object@pointSamples[(object@warmup+1):(object@samples+1),1,which.par.x]),
       as.vector(object@pointSamples[(object@warmup+1):(object@samples+1),1,which.par.y]),
       xlab=which.par.x,ylab=which.par.y,pch=20,cex=0.5,col=1)

  if(object@chains>1){
    for(i in 1:object@chains){
      points(as.vector(object@pointSamples[(object@warmup+1):(object@samples+1),i,which.par.x]),
             as.vector(object@pointSamples[(object@warmup+1):(object@samples+1),i,which.par.y]),
             xlab=which.par.x,ylab=which.par.y,pch=20,cex=0.5,col=i)
    }
  }
...
getSample<-function(object,which.par=NULL,include.warmup=FALSE){
  ...
  colnames(ret) <- colnames(object@pointSamples[,1,])
...
setMethod(f="show",
          ...
          definition = function(object){
            ...
            if(max(dim(object@intSamples))>0){
            ...
            message("NOTE: integrated samples do NOT reflect the complete target distibution, only indicated moments with respect to the target distribution")
              ...
              message("No integrated samples recorded.")
            }
            return(0)
          })
